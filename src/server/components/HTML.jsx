...
export default function HTML({ hostname, url, isLogin, children, title, description, keywords, image, css, scripts, state, csrf }) {
  // Render
  return (
    <html>
      <head>
        ...
        <meta name="google-site-verification" content="xiMe8anTjJm9_X1C2X-DqdjN7zXptepvd9lILEC130M" />
        ...
        <meta property="og:description" content={description || process.env.APP_DESCRIPTION} />
        <meta property="og:image" content={image ? image : `${hostname}/${configBuild.folderAssets}/l.png`} />
      ...
      <body>
        ...
        />
        {scripts.map(({ async, src }) => (async ? <script key={src} crossOrigin="true" async src={src} /> : <script key={src} src={src} />))}
      </body>
    </html>
  )
}
